...
- name: Add a hostname entry to /etc/hosts
  theurbanpenguin.utils.manage_hosts
    hostname: "server1"
    ip: "192.168.1.10"
    state: present
    ...
    try:
                ...
                if line_hostname.lower() == hostname.lower():
                    ...
                    if line_ip == ip:
                        ...
                        if state == "present":
                            ...
                            result["changed"] = True
                            result["message"] = "IPv4 {ip} updated for hostname."
        ...
        if state == "present" and not found_hostname:
            if not module.check_mode:
                updated_lines.append(f"{ip}\t{hostname}\n")
            result["changed"] = True
            result["message"] = f"FQDN Hostname {hostname} added with IPv4 {ip}."
        ...
        if state == "absent" and found_hostname:
            ...
            result["changed"] = True
            result["message"] = f"FQDN Hostname {hostname} removed."
